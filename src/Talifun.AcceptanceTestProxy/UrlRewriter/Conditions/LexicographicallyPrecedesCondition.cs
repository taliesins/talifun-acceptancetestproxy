...
namespace Talifun.AcceptanceTestProxy.UrlRewriter.Conditions
{
	...
	/// 
	/// </summary>
	public class LexicographicallyPrecedesCondition : ICondition
	{
		...
		public bool Evaluate(ConditionContext context)
		{
			...
			var pattern = Pattern.ToString().Substring(1);

			var comparison = string.Compare(test, pattern, ConditionFlagsProcessor.HasNoCase(context.ConditionFlags)) < 0;
