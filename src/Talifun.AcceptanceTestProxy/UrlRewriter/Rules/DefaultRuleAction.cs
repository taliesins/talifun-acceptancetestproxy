...
namespace Talifun.AcceptanceTestProxy.UrlRewriter.Rules
{
	...
	{
        private static readonly ILog Logger = LogManager.GetLogger(typeof(DefaultRuleAction));
		...
		public DefaultRuleAction(string pattern, string substitution)
			: this(pattern, substitution, UrlRewriterManager.RuleOptions) { }
		...
		{
			var testUrl = context.CurrentUrl.ToString();
		    Logger.InfoFormat("Input: {0}",testUrl);
		...
		{
			var inputUrl = context.CurrentUrl.ToString();
			var substituedUrl = Pattern.Replace(inputUrl, _substitution, context);

            Logger.InfoFormat("Output: {0}", substituedUrl);
