...
namespace Talifun.AcceptanceTestProxy.UrlRewriter.Rules.Flags
{
	...
	{
        private static readonly ILog Logger = LogManager.GetLogger(typeof(ResponseStatusFlag));
		...
		public ResponseStatusFlag(string type)
		{
			...
			{
                Logger.Error("Could not create a response status from " + type, ex);
			}
		...
		public RuleFlagProcessorResponse Apply(RuleContext context)
		{
			if (StatusCode > 0U)
			{
                ...
                Logger.InfoFormat("HTTP Status: {0}.{1} {2} ", response.StatusCode, response.SubStatusCode, response.StatusDescription);
			}
